...
module.exports = {
  alias: ["rn"],
  description: "Renames a React Native app",
  run: async (toolbox: GluegunToolbox) => {
    ...
    const renameConfirmation = await prompt.ask({
      type: "input",
      name: "confirmation",
      message: `Are you sure you want to rename your app (y/N)?`,
    })
    if (renameConfirmation.confirmation !== "y" && renameConfirmation.confirmation !== "Y") {
      warning("Rename aborted. There's no changes applied")
      return
    }
